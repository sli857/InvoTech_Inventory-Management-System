 ...
 * It fetches all audits from a backend service on an initial load and provides an interface for audit filtering.
 * @return {JSX.Element} The IMSAudits component.
 */
function IMSAudits() {
  ...
  /**
     ...
     */
  useEffect(() => {
    handleFetchAudits(`${backendBaseurl}/audits`);
  }, []);
              ...
              <Form>
                <Row>
                  <Col sm={4}>
                    ...
                    <Button className="mt-2" onClick={() => handleFetchAudits(backendBaseurl+`/audits/onTable?tableName=${selectedTable}`)}>Filter by Table</Button>
                  </Col>
                  <Col sm={6}>
                    <DatePicker className="form-control" selected={startDate} onChange={(date) => setStartDate(date)} />
                    <DatePicker className="form-control mt-2" selected={endDate} onChange={(date) => setEndDate(date)} />
                    <Button className="mt-2" onClick={() => handleFetchAudits(backendBaseurl+`/audits/betweenPeriod?start=${startDate.toISOString().slice(0, 10)}&end=${endDate.toISOString().slice(0, 10)}`)}>Filter by Date</Button>
                  </Col>
                </Row>
              </Form>
            </Card.Body>
