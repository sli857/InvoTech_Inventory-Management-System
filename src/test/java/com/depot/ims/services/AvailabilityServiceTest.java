...
/**
 * Unit tests for the AvailabilityService class. Validates the functionality of availability
 * management operations, including retrieval, and adding of availability details. Utilizes Mockito
 * to mock the siteRepository, ItemRepository, AvailabilityRepository for isolated testing of
 * service logic.
 */
class AvailabilityServiceTest {

  @Mock private SiteRepository siteRepository = mock(SiteRepository.class);
  @Mock private ItemRepository itemRepository = mock(ItemRepository.class);
  @Mock private AvailabilityRepository availabilityRepository = mock(AvailabilityRepository.class);
  @Mock private AuditService auditService = mock(AuditService.class);
  private final AvailabilityService availabilityService =
      new AvailabilityService(siteRepository, itemRepository, availabilityRepository, auditService);

  /**
   * Tests adding an availability mock the availability repository Verifies correct return of the
   * addAvailabilities method
   */
  @Test
  void TestAddAvailabilities() {
    ...
    assertEquals(response1.getBody(), availability);
  }

  /**
   * Tests retrieving an availability mock the availability repository and site repository Verifies
   * the correct return of the addAvailability method
   */
  @Test
  void TestGetAvailability() {
    ...
    assertEquals(response3.getBody(), list);
  }

  /**
   * Tests retrieving a list of availability by item id mock the availability repository and item
   * repository Verifies the correct return of the getAvailabilitiesByItemId method
   */
  @Test
  void getAvailabilitiesByItemId() {
    Item item = new Item(20L, "item1", 3.0);
    Item item1 = new Item(30L, "item2", 4.0);
    Site site = new Site(11L, "HomeDepot 2", "W54 N53", "open", null, true);
    ...
    assertEquals(response4.getBody(), list);
  }

  /**
   * Tests retrieving a list of availability by item id and site id mock the availability
   * repository, item repository, and site repository Verifies correct return of the
   * getAvailabilityBySiteIdAndItemId method
   */
  @Test
  void TestGetAvailabilityBySiteIdAndItemId() {
    Item item = new Item(20L, "item1", 3.0);
    Item item1 = new Item(30L, "item2", 4.0);
    Site site = new Site(11L, "HomeDepot 2", "W54 N53", "open", null, true);
    Site site1 = new Site(12L, "HomeDepot 3", "W54 N53", "open", null, true);
    ...
    ResponseEntity<?> response5 =
        this.availabilityService.getAvailabilityBySiteIdAndItemId(11L, 20L);
    assertEquals(HttpStatus.OK, response5.getStatusCode());
    assertNotNull(response5.getBody());
    assertEquals(response5.getBody(), availability);
  }

  /**
   * Tests retrieving a list of sites by a list of items mock the availability repository, item
   * repository, and site repository Verifies the correct return of the getSitesByItems method
   */
  @Test
  void getSitesByItems() {
    ...
    assertEquals(response6.getBody(), expectedList);
    ...
    assertEquals(response7.getBody(), siteList);
  }

  /**
   * Tests changing the quantity of an item in a site mock the availability repository, item
   * repository, and site repository Verifies the correct return of the changeQuantity method
   */
  @Test
  void TestChangeQuantity() {

    Item item1 = new Item(1L, "item1", 3.0);
    Site site1 = new Site(13L, "HomeDepot 2", "W54 N53", "open", null, true);
    ...
    assertEquals(response8.getBody(), updatedAvailability);
  }
}
