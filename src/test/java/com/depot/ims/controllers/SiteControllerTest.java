...
public class SiteControllerTest {
  @InjectMocks SiteController siteControllerMock;
  @Mock SiteRepository siteRepositoryMock;
  @Mock SiteService siteServiceMock;
  ...
  void testGetSiteStatus() throws Exception {
    ...
    ResponseEntity<?> res = new ResponseEntity<>("open", null, HttpStatus.OK);
    doReturn(res).when(siteServiceMock).getStatusBySiteId(any());
    mockMvc
        .perform(get("/sites/status?siteId=1"))
        .andExpect(status().isOk())
  ...
  void testGetSiteBySiteId() throws Exception {
    ...
    ResponseEntity<Site> res1 = new ResponseEntity<>(site1, null, HttpStatus.OK);
    ResponseEntity<Site> res2 = new ResponseEntity<>(site2, null, HttpStatus.OK);

    doReturn(res1).when(siteServiceMock).getSite(site1Id, null);
    doReturn(res2).when(siteServiceMock).getSite(null, site2Name);

    mockMvc
        .perform(get("/sites/site?siteId=" + site1Id))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").value(site1));

    mockMvc
        .perform(get("/sites/site?siteName=" + site2Name))
        .andExpect(status().isOk())
  ...
  void testDelete() throws Exception {
    ...
    mockMvc
        .perform(delete("/sites/delete?siteId=1&ceaseDate=2024-4-5"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.siteStatus").value("closed"));

    mockMvc
        .perform(delete("/sites/delete?siteId=2"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.siteStatus").value("closed"));
  }
}
