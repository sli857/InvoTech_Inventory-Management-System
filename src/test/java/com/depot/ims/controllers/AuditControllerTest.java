...
public class AuditControllerTest {
  @InjectMocks AuditController auditControllerMock;
  @Mock AuditService auditServiceMock;
  @Mock AuditRepository auditRepositoryMock;
  ...
  void testFindAuditsOnTable() throws Exception {
    User user1 = User.builder().username("user1").password("pass").position("admin").build();
    User user2 = User.builder().username("user2").password("pass").position("admin").build();
    ...
    mockMvc
        .perform(get("/audits/onTable?tableName=sites"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
  ...
  void testFindAuditsBetweenDates() throws Exception {
    ...
    doReturn(ResponseEntity.ok(result))
        .when(auditServiceMock)
        .findAuditsBetweenPeriod("2024-02-01", "2024-04-01");

    mockMvc
        .perform(get("/audits/betweenPeriod?start=2024-02-01&end=2024-04-01"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
        .andExpect(jsonPath("$.length()").value(2));
  }
