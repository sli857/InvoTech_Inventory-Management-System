...
/**
 * Tests for ItemController - ensures correct handling of user-related requests. Mocks the
 * ItemRepository and ItemService to isolate controller logic and verify interactions.
 */
public class ItemControllerTest {
  ...
  /**
   * Tests retrieval of all items, verifying correct HTTP status and JSON structure. Mocks
   * ItemsRepository to return a predefined list of item. Asserts the size of the returned item list
  ...
  void addItem() throws Exception {
    ...
    String jsonBody =
        "{" + "\"itemId\": 2," + "\"itemName\": \"name\"," + "\"itemPrice\": 1.1" + "}";
    mockMvc
        .perform(post("/items/add").contentType(MediaType.APPLICATION_JSON).content(jsonBody))
        .andExpect(status().isOk());
  }

  /**
   * Tests retrieval of an item by item id or item name, verifying correct HTTP status and JSON
   * structure.
   */
  @Test
