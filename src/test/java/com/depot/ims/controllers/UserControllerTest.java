...
/**
 * Tests for UserController - ensures correct handling of user-related requests. Mocks the
 * UserRepository and UserService to isolate controller logic and verify interactions.
 */
public class UserControllerTest {
  ...
  /**
   * Tests retrieval of all users, verifying correct HTTP status and JSON structure. Mocks
   * UserRepository to return a predefined list of users. Asserts the size of the returned user list
   * and specific user attributes.
   */
  @Test
  ...
  /**
   * Tests retrieval of a single user by ID, verifying correct HTTP status and JSON structure. Mocks
   * UserService to return a specific user for a given ID. Asserts the attributes of the returned
   * user.
   */
  @Test
  public void testGetUser() throws Exception {
    ...
    ResponseEntity<User> res = new ResponseEntity<>(user, null, HttpStatus.OK);

    doReturn(res).when(userService).getUser(1L, null);

    // Perform GET request to /user endpoint with userId as request parameter
    mockMvc
        .perform(get("/users/user").param("userId", "1").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  public void testAddUser() throws Exception {
    ...
    // Perform the test and verify results
    mockMvc
        .perform(post("/users/add").contentType(MediaType.APPLICATION_JSON).content(userJson))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.userId", is(1)))
        .andExpect(jsonPath("$.username", is("user1")))
        .andExpect(jsonPath("$.password", is("password1")))
  ...
  /**
   * Tests updating an existing user, verifying the correct HTTP status. Mocks UserService to
   * simulate the update operation and verify the update request's attributes.
   */
  @Test
  ...
  /**
   * Tests the attempt to delete a non-existing user, verifying the correct HTTP status and error
   * message. Mocks UserService to simulate the case where the user does not exist.
   */
  @Test
