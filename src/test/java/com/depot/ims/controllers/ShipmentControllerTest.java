...
/**
 * Tests for the ShipmentController class. These tests mock the behavior of the ShipmentService and
 * ShipmentRepository to verify the ShipmentController's request mappings, response status codes,
 * and body contents.
 */
public class ShipmentControllerTest {
  @InjectMocks ShipmentController shipmentController;
  ...
  /**
   * Sets up the testing environment before each test. Initializes mocks and configures the MockMvc
   * object for testing the controller.
   */
  @BeforeEach
  public void setup() {
  ...
  /**
   * Test to verify the retrieval of all shipments. Checks for the correct status code and the
   * presence of shipment data in the response.
   */
  @Test
  public void testGetShipments() throws Exception {
    ...
    mockMvc
        .perform(get("/shipments").contentType(MediaType.APPLICATION_JSON))
  ...
  /**
   * Test to verify the retrieval of a single shipment by its ID. Checks for correct status code and
   * shipment details in the response.
   */
  @Test
  public void testGetShipment() throws Exception {
    ...
    ResponseEntity<Shipment> res = new ResponseEntity<>(shipment, null, HttpStatus.OK);
  ...
  /**
   * Test to verify the addition of a new shipment. Checks for correct status code and newly added
   * shipment details in the response.
   */
  @Test
  public void testAddShipment() throws Exception {
    ...
    mockMvc
        .perform(
            post("/shipments/add").contentType(MediaType.APPLICATION_JSON).content(shipmentJson))
        .andExpect(status().isOk())
  ...
  /**
   * Test to verify the update functionality for a shipment. Checks for correct status code and the
   * expected success message in the response.
   */
  @Test
  public void testUpdateShipment() throws Exception {
    ...
    doReturn(ResponseEntity.ok("Successfully updated"))
  ...
  /**
   * Test to verify successful deletion of a shipment. Checks for correct status code and the
   * expected success message in the response.
   */
  @Test
  void deleteShipmentFound() throws Exception {
    // Mock the service call to return a success response
    doReturn(ResponseEntity.ok("Successfully deleted")).when(shipmentService).deleteShipment(1L);

    // Perform the test and verify results
    mockMvc
        .perform(
            delete("/shipments/delete").param("shipmentId", "1").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  /**
   * Test to verify the behavior when attempting to delete a non-existent shipment. Checks for
   * correct status code and error message in the response.
   */
  @Test
  void deleteShipmentNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/shipments/delete").param("shipmentId", "2").accept(MediaType.APPLICATION_JSON))
